...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='CE', command=partial(self.borrarPanel)).grid(column=2, row=8, sticky=W)
    ...
    def ponerNUMERO(self, numero):
        if self.__operadorAux==None:
            ...
            if "/" in valor:
                split = valor.split("/")
                self.__primerOperando=Fraccion(int(split[0]),int(split[1]))
    ...
    def ponerOPERADOR(self, op):
        if op=='=':
            ...
            segundo_operando = (self.__panel.get())
            if "/" in segundo_operando:
                split = segundo_operando.split("/")
                self.__segundoOperando=Fraccion(int(split[0]),int(split[1]))
            else:
